...
public class DDOServer extends Thread implements CenterServer {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread threadSocket = new Thread(ddo);
		threadSocket.start();

		try {
			Endpoint endPoint = Endpoint.publish("http://localhost:3456/ws/Dollard", ddo);
			if (endPoint.isPublished()) {
				System.out.println("Dollard server published successfully");
			}
			...
			while (true) {
				...
				if (requestContent.startsWith("ST") || requestContent.startsWith("TR")) {
					...
					case "ST":
						...
						// adding the operation to the log file
						ddoLogger.mLogger.info(" New Student record transfered with values: " + objRecord + '\n');
					...
					case "TR":
						...
						// adding the operation to the log file
						ddoLogger.mLogger.info(" New Teacher record transfered with values: " + objRecord + '\n');
						break;
					}
					...
					ddoLogger.mLogger.info(" DDO server received request to send '\n'");
