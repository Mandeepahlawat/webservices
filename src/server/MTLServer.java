...
public class MTLServer extends Thread implements CenterServer {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread threadSocket = new Thread(mtl);
		threadSocket.start();

		try {
			Endpoint endPoint = Endpoint.publish("http://localhost:1234/ws/Montreal", mtl);
			if (endPoint.isPublished()) {
				System.out.println("Montreal server published successfully");
			}
