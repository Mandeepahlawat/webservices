...
public class ManagerClient extends Thread {
	...
	public void fetchServer(String serverName) {
		...
		if (serverName.equals("mtl")) {
			MTLServerService objService = new MTLServerService();
			serverImpl = objService.getMTLServerPort();
		} else if (serverName.equals("lvl")) {
			LVLServerService objService = new LVLServerService();
			serverImpl = objService.getLVLServerPort();
		} else if (serverName.equals("ddo")) {
			DDOServerService objService = new DDOServerService();
			serverImpl = objService.getDDOServerPort();
		} else {
	...
	public String courseMenu(Scanner scan, StringBuilder sbCourseList) {
		...
		try {
			...
			do {
				...
				System.out.println("Select another Course\n" + "1. Maths\n" + "2. French\n" + "3. Science\n" + "4. Exit");
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (inputFromUser == 1) 	 {	client.normalMode();	}
			else if (inputFromUser == 2) {	client.testMode();		}
